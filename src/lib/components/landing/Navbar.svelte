...
<div class="navbar-home top-0 py-2 lg:py-5 w-full bg-transparent lg:relative z-50">
	<nav class="z-10 sticky top-0 left-0 right-0 max-w-4xl xl:max-w-5xl mx-auto px-5 py-3 lg:border-none lg:py-4">
		<div class="flex items-center justify-between">
			<button>
				<div class="header-logo flex items-center space-x-2">
					<h2 class="text-black font-bold text-2xl">Incln</h2>
				</div>
			</button>
			<div class="header-menu hidden lg:block">
				<ul class="flex space-x-10 text-sm font-semibold text-zinc-700/60">
					{#each menuItems as menuItem}
						<li
							class="w-fit border-b border-transparent hover:border-zinc-900/90 transition-colors duration-100 ease-linear hover:text-zinc-900/90"
						>
							<a href={menuItem.url}>
				...
				>
					{#if isMobileMenuOpen}
						<svg
							...
							class="text-2xl focus:outline-none active:scale-110 active:text-slate-200"
							width="1em"
							height="1em"
							viewBox="0 0 24 24"
							><path
	...
	{#if isMobileMenuOpen}
		...
		>
			<div
				class="main-nav-menu-mobile z-50 bg-white/70 backdrop-blur-lg shadow-sm lg:shadow-none absolute left-0 top-0 bottom-0 right-0 border-r border-slate-400/30 border-dashed w-[270px] md:w-[300px] lg:flex lg:w-[300px] lg:flex-col"
			>
				<button>
					<div class="header-logo p-5 flex items-center space-x-2">
						<h2 class="text-black font-bold text-2xl">Incln</h2>
					</div>
				</button>
