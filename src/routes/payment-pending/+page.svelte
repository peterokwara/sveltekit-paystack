<script lang="js">
    ...
    let currentStatus = $state(data.paymentDetails?.status || "pending");
    ...
    async function checkPaymentStatus() {
        ...
        try {
            ...
            const response = await fetch(
                `/api/payments/paystack/status/${referenceId}`,
            );
            if (!response.ok) {
    ...
    $effect(() => {
        if (currentStatus === "completed") {
            ...
            setTimeout(
                () => goto("/checkout?status=failed", { replaceState: true }),
                3000,
            );
        }
    });
        ...
        <div class="w-full max-w-md text-center">
            {#if currentStatus === "pending"}
                <div>
                    ...
                    <p class="mt-4 text-base text-gray-500">
                        ...
                        take a moment. Do not close this page.
                    </p>
                </div>
            {:else if currentStatus === "completed"}
                <div>
                    ...
                    <p class="mt-4 text-base text-gray-500">
                        Your payment has been confirmed. You will be redirected
                        shortly.
                    </p>
                </div>
            {:else if currentStatus === "failed"}
                <div>
                    ...
                    <p class="mt-4 text-base text-gray-500">
                        We could not confirm your payment. You will be
                        redirected to try again.
                    </p>
                </div>
            {/if}
        </div>
